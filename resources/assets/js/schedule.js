...
function eventDropFunc(event, delta, revertFunc, jsEvent, ui, view) {
  ...
  var action = $("#modalForm").attr("action", `//${location.hostname}/staffs/${dataApoId}`);
...
function setCalendarList(startDate, endDate, timezone, callback) {
  axios({
    method: "get",
    url: `//${location.hostname}/appointments`,
    params: {
      start: startDate,
      end: endDate,
      timezone: timezone,
...
$(function () {
  ...
  containerEl.fullCalendar({
    ...
    eventSources: [
      ...
      // },
      {
        url: `//${location.hostname}/holidays`,
        rendering: 'background',
        color: '#ffd0d0',
        cache: true
      }
